...
export async function createOrder(username: string, idCesta: string) {
  ...
  let orderId: number = 0; // por si algo falla
  try {
    await db.run("BEGIN TRANSACTION");

    // Aseguramos que el customer existe
    await ensureCustomer(username);
    ...
    if (!customer) {
      console.error("createOrder: customer no encontrado para", username);
      await db.run("ROLLBACK");
      return { orderId: 0, totalAmount: 0 };
    }
    ...
    orderId = result.lastID ?? 0;

    // 2) Obtenemos los productos de la cesta SOLO por idCesta
    const cestaItems = (await db.all(
      `
      SELECT c.ProductID, c.cantidad, p.UnitPrice
      FROM cesta c
    ...
    `,
      [idCesta]
    )) as { ProductID: number; cantidad: number; UnitPrice: number }[];

    if (!cestaItems || cestaItems.length === 0) {
      console.error(
        "createOrder: cesta vacía o no encontrada para idCesta",
        idCesta
      );
      await db.run("ROLLBACK");
      return { orderId: 0, totalAmount: 0 };
    ...
    // 3) Insertamos las líneas de detalle
    ...
    // 4) Calculamos el total del pedido
    ...
    // 5) Limpiamos la cesta de ese idCesta
    ...
    await db.run("ROLLBACK");
    console.error("Error creating order:", error);
    // NO lanzamos error → devolvemos “pedido vacío”
    return { orderId: 0, totalAmount: 0 };
  }
}
