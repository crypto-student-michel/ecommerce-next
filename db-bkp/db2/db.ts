...
async function ensureCestaTable(db: Database<sqlite3.Database, sqlite3.Statement>) {
  await db.exec(`
...
// alias (por si en algún punto usas getProductById)
...
export async function getUser(username: string, password: string) {
  ...
  const token = jwt.sign({ id: (user as any).id, username: (user as any).username }, secret, {
    expiresIn: "1h",
  });
...
export async function getCustomerOrders(customerId: string) {
  ...
  return db.all(
    ...
    SELECT
      ...
      (SELECT CASE WHEN COUNT(*) > 0 THEN 1 ELSE 0 END FROM cobro WHERE orderId = Orders.OrderID) AS Cobrado
    FROM Orders
    WHERE CustomerID = ?
...
export async function getCesta(idCesta: string) {
  ...
  return db.all(
    `
    SELECT c.productId, p.ProductName AS productName, c.cantidad
    FROM cesta c
